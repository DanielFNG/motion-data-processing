...
processStaticData(test_dir, raw_static, marker_rotations);
...
processMarkerData(test_dir, raw_markers, marker_rotations, speed, direction);
assert(Data([test_dir filesep markers]) == ...
    Data([true_dir filesep 'MarkersComp' filesep markers]));

%% Test 4: Markers with segmentation
processMarkerData(test_dir, raw_markers, marker_rotations, [], [], ...
    feet, kin_mode, [], marker_dir);
...
%% Test 6: GRF only
processGRFData(test_dir, raw_grfs, grf_rotations, []);
assert(Data([test_dir filesep mot]) == ...
    Data([true_dir filesep grf_dir filesep mot]));

%% Test 7: GRF with motion compensation
processGRFData(test_dir, raw_grfs, grf_rotations, speed, direction);
assert(Data([test_dir filesep mot]) == ...
...
processMotionData(test_dir, test_dir, raw_markers, raw_grfs, ...
    marker_rotations, grf_rotations, time_delay, []);
assert(Data([test_dir filesep markers]) == ...
...
processMotionData(test_dir, test_dir, raw_markers, raw_grfs, ...
    marker_rotations, grf_rotations, time_delay, speed, direction);
assert(Data([test_dir filesep markers]) == ...
...
processMotionData(test_dir, test_dir, raw_markers, raw_grfs, ...
    marker_rotations, grf_rotations, time_delay, [], [], ...
    feet, kin_mode, [], marker_dir, grf_dir);
compareFolder([test_dir filesep side filesep grf_dir], ...
...
processMotionData(test_dir, test_dir, raw_markers, raw_grfs, ...
    marker_rotations, grf_rotations, time_delay, [], [], ...
    feet, grf_mode, cutoff, marker_dir, grf_dir);
compareFolder([test_dir filesep side filesep grf_dir], ...
