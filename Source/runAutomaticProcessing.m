...
%% Setup parameters.
input_folder = 'C:\Users\danie\Documents\GitHub\motion-data-processing\Source\test_folder';  % Folder containing .TRC and .TXT data.
...
save_dir = '';  % Directory in which resultant fils are saved. 
...
for i=1:n_files
    ...
    input_markers = [input folder filesep marker_files(i).name];
    output_markers = [save_dir filesep marker_files(i).name];
    output_grf = produceMOT(input_grf, output_grf);
    [markers, grfs] = synchronise(input_grf, input_markers, time_delay);
