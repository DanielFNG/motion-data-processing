function [RHJC, LHJC] = getHJCHarrington(markers)
% Not sure if this file originated with Graham or if it's from MoTONMS. I
% have made some slight changes.
...
LASIS=transpose([markers.getColumn('L_ASIS_X'), ...
    markers.getColumn('L_ASIS_Y'), ...
    markers.getColumn('L_ASIS_Z')]);  
RASIS=transpose([markers.getColumn('R_ASIS_X'), ...
    markers.getColumn('R_ASIS_Y'), ...
    markers.getColumn('R_ASIS_Z')]);  
SACRUM=transpose([markers.getColumn('V_Sacral_X'), ...
    markers.getColumn('V_Sacral_Y'), ...
    markers.getColumn('V_Sacral_Z')]);

% Changed sacrum calculation

for t=1:size(RASIS,2)

    %Right-handed Pelvis reference system definition 
    %%RPSIS(2,t)= RPSIS(2,t)- 100; %%Modification for wearing the APO
    %SACRUM(:,t)=(RPSIS(:,t));      
